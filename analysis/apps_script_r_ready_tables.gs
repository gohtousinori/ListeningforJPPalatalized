...
function doPost(e) {
  ...
  try {
    ...
    const respondent = normalizeRespondentData_(payload, submissionId);
    payload.respondent_data = respondent;
...
function normalizeRespondentData_(payload, submissionId) {
  var respondent = cloneObject_((payload && payload.respondent_data) || {});
  var snapshot = (payload && payload.pre_survey_snapshot && typeof payload.pre_survey_snapshot === 'object')
    ? payload.pre_survey_snapshot
    : {};

  // Merge snapshot values into respondent when respondent is missing values.
  var snapshotKeys = Object.keys(snapshot);
  for (var i = 0; i < snapshotKeys.length; i++) {
    var key = snapshotKeys[i];
    if (isBlank_(respondent[key])) {
      respondent[key] = snapshot[key];
    }
  }

  var canonical = {
    email: firstNonEmpty_([
      respondent.email,
      respondent.Email,
      snapshot.email,
      pickValueByRegex_(respondent, [/^email$/i])
    ]),
    native_language: firstNonEmpty_([
      respondent.native_language,
      snapshot.native_language,
      pickValueByRegex_(respondent, [/^母語 \/ native language$/i, /native language/i])
    ]),
    japanese_language_order: firstNonEmpty_([
      respondent.japanese_language_order,
      snapshot.japanese_language_order,
      pickValueByRegex_(respondent, [/japanese language order/i, /日本語は第何言語/i])
    ]),
    total_languages_learned: firstNonEmpty_([
      respondent.total_languages_learned,
      snapshot.total_languages_learned,
      pickValueByRegex_(respondent, [/total number of languages learned/i, /学習経験のある言語数/i])
    ]),
    learned_languages_list: firstNonEmpty_([
      respondent.learned_languages_list,
      snapshot.learned_languages_list,
      pickValueByRegex_(respondent, [/learned languages list/i, /学習言語リスト/i])
    ]),
    phonology_theory_experience: firstNonEmpty_([
      respondent.phonology_theory_experience,
      snapshot.phonology_theory_experience,
      pickValueByRegex_(respondent, [/phonology.*phonetics.*experience/i, /音韻論.*音声学.*経験/i])
    ]),
    study_year: firstNonEmpty_([
      respondent.study_year,
      snapshot.study_year,
      pickValueByRegex_(respondent, [/^日本語学習期間（年）/i, /jp learning years/i, /^study_year$/i])
    ]),
    study_month: firstNonEmpty_([
      respondent.study_month,
      snapshot.study_month,
      pickValueByRegex_(respondent, [/^日本語学習期間（月）/i, /jp learning months/i, /^study_month$/i])
    ]),
    stay_year: firstNonEmpty_([
      respondent.stay_year,
      snapshot.stay_year,
      pickValueByRegex_(respondent, [/^日本滞在期間（年）/i, /years lived in japan/i, /^stay_year$/i])
    ]),
    stay_month: firstNonEmpty_([
      respondent.stay_month,
      snapshot.stay_month,
      pickValueByRegex_(respondent, [/^日本滞在期間（月）/i, /months lived in japan/i, /^stay_month$/i])
    ]),
    pron_experience: firstNonEmpty_([
      respondent.pron_experience,
      snapshot.pron_experience,
      pickValueByRegex_(respondent, [/pronunciation learning experience/i, /発音学習経験/i])
    ]),
    institution: firstNonEmpty_([
      respondent.institution,
      snapshot.institution,
      pickValueByRegex_(respondent, [/institution/i, /受講機関/i])
    ])
  };

  respondent.submission_id = String(submissionId || respondent.submission_id || '').trim();
  respondent.submitted_at = firstNonEmpty_([
    respondent.submitted_at,
    (payload && payload.submitted_at) || ''
  ]) || new Date().toISOString();

  respondent.email = canonical.email;
  respondent.Email = canonical.email;
  respondent.native_language = canonical.native_language;
  respondent.japanese_language_order = canonical.japanese_language_order;
  respondent.total_languages_learned = canonical.total_languages_learned;
  respondent.learned_languages_list = canonical.learned_languages_list;
  respondent.phonology_theory_experience = canonical.phonology_theory_experience;
  respondent.study_year = canonical.study_year;
  respondent.study_month = canonical.study_month;
  respondent.stay_year = canonical.stay_year;
  respondent.stay_month = canonical.stay_month;
  respondent.pron_experience = canonical.pron_experience;
  respondent.institution = canonical.institution;

  // Keep Japanese/English label columns populated for Summary compatibility.
  respondent['母語 / Native language'] = firstNonEmpty_([
    respondent['母語 / Native language'],
    canonical.native_language
  ]);
  respondent['日本語は第何言語か（300時間以上） / Japanese language order (300+ study hours)'] = firstNonEmpty_([
    respondent['日本語は第何言語か（300時間以上） / Japanese language order (300+ study hours)'],
    canonical.japanese_language_order
  ]);
  respondent['学習経験のある言語数（合計） / Total number of languages learned'] = firstNonEmpty_([
    respondent['学習経験のある言語数（合計） / Total number of languages learned'],
    canonical.total_languages_learned
  ]);
  respondent['学習言語リスト（母語含む、学習順） / Learned languages list (including L1, in study order)'] = firstNonEmpty_([
    respondent['学習言語リスト（母語含む、学習順） / Learned languages list (including L1, in study order)'],
    canonical.learned_languages_list
  ]);
  respondent['音韻論・音声学（理論）学習経験 / Academic-theoretical phonology/phonetics experience'] = firstNonEmpty_([
    respondent['音韻論・音声学（理論）学習経験 / Academic-theoretical phonology/phonetics experience'],
    canonical.phonology_theory_experience
  ]);
  respondent['日本語学習期間（年） / JP learning years'] = firstNonEmpty_([
    respondent['日本語学習期間（年） / JP learning years'],
    canonical.study_year
  ]);
  respondent['日本語学習期間（月） / JP learning months'] = firstNonEmpty_([
    respondent['日本語学習期間（月） / JP learning months'],
    canonical.study_month
  ]);
  respondent['日本滞在期間（年） / Years lived in Japan'] = firstNonEmpty_([
    respondent['日本滞在期間（年） / Years lived in Japan'],
    canonical.stay_year
  ]);
  respondent['日本滞在期間（月） / Months lived in Japan'] = firstNonEmpty_([
    respondent['日本滞在期間（月） / Months lived in Japan'],
    canonical.stay_month
  ]);
  respondent['発音学習経験 / Pronunciation learning experience'] = firstNonEmpty_([
    respondent['発音学習経験 / Pronunciation learning experience'],
    canonical.pron_experience
  ]);
  respondent['受講機関 / Institution'] = firstNonEmpty_([
    respondent['受講機関 / Institution'],
